...
class App(resources.AppSkeleton):
    ...
    def get_eeg_worker_name(self, working_lsl_streams_info):
        for lsl_info in working_lsl_streams_info:
            if lsl_info['medusa_type'] == 'EEG':
                self.send_to_log(
                    'Linked to EEG stream: %s' % lsl_info['lsl_name'])
            ...
            if self.currently_processing_onset_idx is not None:
                ...
                if self.check_processing_feasibility(
                    ...
                    threading.Thread(
                        ...
                        name="pary_cvep_speller_decoding"
                    ).start()
    ...
    def process_trial(self, last_onset_idx=None):
        ...
        try:
            ...
            # TODO: A veces entra aquí pero no tiene ningún sentido. Se hace
            #  un check_predict_feasibility antes de iniciar este thread. El
            #  caso es que pasa el check tanto antes como ahora, pero en el
            #  predict no. Es cierto que el timestamp+window es mayor que el
            #  EEG, pero entonces por qué se pasa el check antes y no en el
            #  predict??
            print("> CHECK: %s" % str(self.cvep_model.check_predict_feasibility_signal(
            times=times_, onsets=self.cvep_data.onsets[:last_onset_idx + 1],
            fs=fs)))
            print("> STILL ERROR")
            print("Times: " + str(last_onset_idx+1) + "-" + str(times_[-1]) +
                  "-" + str(times_[-1] + self.cvep_model.get_inst('clf_method').fitted[
                      "len_epoch_ms"]))
            print("")
