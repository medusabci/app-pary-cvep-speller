...
class Config(QtWidgets.QDialog, ui_main_file):
    ...
    def update_encoding_info(self):
        ...
        seqlen = base ** order - 1
        duration = seqlen / self.spinBox_fpsresolution.value()
        self.lineEdit_length.setText(str(seqlen))
    ...
    def set_settings_to_gui(self):
        ...
        if len(self.settings.colors.color_point) == 9:
            if self.settings.colors.color_point[-2:] == '00':
                self.radio_show_point.setChecked(False)

        # Variable colors
        self.tableWidget_color_sequences.clear()
        self.tableWidget_color_sequences.setColumnCount(3)
        ...
        for event in self.settings.colors.color_box_dict.keys():
            ...
            self.tableWidget_color_sequences.setCellWidget(idx, 2, btn_text)
            idx += 1
        ...
        for m in range(len(self.settings.matrices['test'])):
            ...
            for r in range(curr_mtx.n_row):
                for c in range(curr_mtx.n_col):
                    ...
                    text_color_ = self.settings.colors.color_text_dict[str(
                        key_)]
                    gui_utils.modify_properties(
                        temp_button, {
    ...
    def get_settings_from_gui(self):
        ...
        if not self.radio_show_point.isChecked():
            self.settings.colors.color_point += '00'  # alpha
        ...
        for i in range(no_colors):
            ...
            btn_text_color = self.tableWidget_color_sequences.cellWidget(i, 2)
            key = int(table_item_event.text())
            box_color = gui_utils.get_property(btn_box_color,
                                               'background-color')
            text_color = gui_utils.get_property(btn_text_color,
                                                'background-color')
    ...
    def update_test_matrix(self):
        ...
        base = int(self.comboBox_base.currentText())
        order = int(self.comboBox_order.currentText())
        mseqlen = int(self.lineEdit_length.text())

        # Compute the matrices
        self.get_settings_from_gui()
        train_matrices, test_matrices, lags_info \
            = self.settings.build_with_pary_sequences(
        ...
        c_text, c_box = settings.Colors.generate_grey_color_dicts(unique)
        self.settings.colors = settings.Colors(
            color_text_dict=c_text, color_box_dict=c_box)
