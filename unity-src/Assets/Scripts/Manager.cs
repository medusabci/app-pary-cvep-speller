...
// P-ary c-VEP Speller (Unity app)
//      > Author: Víctor Martínez-Cagigal

// Versions:
//      - v1.0 (06/07/2022):    P-ary circular-shifting c-VEP speller working
...
public class Manager : MonoBehaviour
{
    ...
    void resetTrainMatrix()
    {
        ...
        for (int r = 0; r < matrixTrain.GetLength(0); r++)
        {
            for (int c = 0; c < matrixTrain.GetLength(1); c++)
            {
                ...
                matrixTrain[r, c].transform.GetChild(1).gameObject.SetActive(false);
            }
        }
    }
    ...
    void resetTestMatrix()
    {
        ...
        for (int r = 0; r < matrixTest.GetLength(0); r++)
        {
            for (int c = 0; c < matrixTest.GetLength(1); c++)
            {
                ...
                matrixTest[r, c].transform.GetChild(1).transform.GetChild(1).gameObject.SetActive(false);
    ...
    void onScreenSizeChange(float width, float height)
    {
        ...
        for (int r = 0; r < matrixTest.GetLength(0); r++)
        {
            ...
            for (int c = 0; c < matrixTest.GetLength(1); c++)
            {
                ...
                RectTransform rt = matrixTest[r, c].transform.GetChild(0).GetComponent<RectTransform>();
                rt.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, cellSize);
                rt.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, cellSize);
                matrixTest[r, c].transform.GetChild(0).transform.position = new Vector2(x_, y_);

                rt = matrixTest[r, c].transform.GetChild(1).GetComponent<RectTransform>();
                rt.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, cellSize);
                rt.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, cellSize);
                matrixTest[r, c].transform.GetChild(1).transform.position = new Vector2(x_, y_);

                // TODO: Maybe the text must not be scaled using cellSize
                rt = matrixTest[r, c].transform.GetChild(1).transform.GetChild(0).GetComponent<RectTransform>();
                rt.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, cellSize);
                rt.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, cellSize);
                matrixTest[r, c].transform.GetChild(1).transform.GetChild(0).transform.position = new Vector2(x_, y_);
    ...
    // MEDUSA updates the probability of selecting each item
    ...
    public void interpretMessage(string message)
    {
        ...
        switch (eventType)
        {
            ...
            case "selection":
                ...
                break;
            case "update_probs":
                // MEDUSA updated the probabilities of selecting each item
    ...
    void onParametersReady()
    {
        ...
        for (int r = 0; r < testNRow; r++)
        {
            ...
            for (int c = 0; c < testNCol; c++)
            {
                ...
                matrixTest[r, c].transform.GetChild(1).transform.GetChild(0).GetComponent<Text>().text = matrices.test[currentMatrixIdx].item_list[idx].text;
                matrixTest[r, c].transform.GetChild(1).transform.GetChild(1).gameObject.SetActive(false);
                matrixTest[r, c].transform.GetChild(1).transform.GetChild(1).GetComponent<Image>().color = hexToColor(parameters.color_point);
        ...
        for (int r = 0; r < trainNRow; r++)
        {
            ...
            for (int c = 0; c < trainNCol; c++)
            {
                ...
                matrixTrain[r, c].transform.GetChild(1).gameObject.SetActive(false);
                matrixTrain[r, c].transform.GetChild(1).GetComponent<Image>().color = hexToColor(parameters.color_point);
    ...
    public void notifyMovedItems()
    {
        ...
        for (int r = 0; r < trainNRow; r++)
        {
            for (int c = 0; c < trainNCol; c++)
            {
                int[] coord = new int[] { currentMatrixIdx, matrices.train[currentMatrixIdx].item_list[idx].row, matrices.train[currentMatrixIdx].item_list[idx].col };
                RectTransform rt = matrixTrain[r, c].GetComponent<RectTransform>();
        ...
        for (int r = 0; r < testNRow; r++)
        {
            for (int c = 0; c < testNCol; c++)
            {
                int[] coord = new int[] { currentMatrixIdx, matrices.test[currentMatrixIdx].item_list[idx].row, matrices.test[currentMatrixIdx].item_list[idx].col };
                RectTransform rt = matrixTest[r, c].transform.GetChild(1).GetComponent<RectTransform>();
                int curr_center_x = (int)(rt.position.x + rt.sizeDelta.x / 2);
                int curr_center_y = Screen.height - (int)(rt.position.y - rt.sizeDelta.y / 2);
                int[] new_pos = estimatePixelPosition(curr_center_x, curr_center_y);
                resizedMatrices.addItem(false, idx, coord, new_pos);
                idx++;
            }
        }
            ...
            {   
                ...
                for (int r = 0; r < matrixTrain.GetLength(0); r++)
                {
                    for (int c = 0; c < matrixTrain.GetLength(1); c++)
                    {
                        ...
                        {
                            matrixTrain[r, c].GetComponent<Image>().color = cellColorsByValue[value];
                            matrixTrain[r, c].transform.GetChild(1).gameObject.SetActive(true);
                            matrixTrain[r, c].transform.GetChild(0).GetComponent<Text>().color = textColorsByValue[value];
                        }
                        else
                        {
            ...
            else
            {
                ...
                for (int r = 0; r < matrixTest.GetLength(0); r++)
                {
                    for (int c = 0; c < matrixTest.GetLength(1); c++)
                    {
                        ...
                        if (cellColorsByValue.ContainsKey(value))
                        {
                            matrixTest[r, c].transform.GetChild(1).GetComponent<Image>().color = cellColorsByValue[value];
                            matrixTest[r, c].transform.GetChild(1).transform.GetChild(1).gameObject.SetActive(true);
                            matrixTest[r, c].transform.GetChild(1).transform.GetChild(0).GetComponent<Text>().color = textColorsByValue[value];
                        }
                        else
                        {
                            Debug.Log("ERROR: The dictionary has not the key " + value.ToString() + "!");
