    ...
    void resetTestMatrix()
    {
        ...
        for (int r = 0; r < matrixTest.GetLength(0); r++)
        {
            for (int c = 0; c < matrixTest.GetLength(1); c++)
            {
                ...
                matrixTest[r, c].transform.GetChild(1).gameObject.SetActive(false);
            }
        }
    } 
    ...
    void onParametersReady()
    {
        ...
        for (int r = 0; r < testNRow; r++)
        {
            ...
            for (int c = 0; c < testNCol; c++)
            {
                ...
                matrixTest[r, c].transform.GetChild(1).gameObject.SetActive(false);
                matrixTest[r, c].transform.GetChild(1).GetComponent<Image>().color = hexToColor(parameters.color_point);
            ...
            else
            {
                ...
                for (int r = 0; r < matrixTest.GetLength(0); r++)
                {
                    for (int c = 0; c < matrixTest.GetLength(1); c++)
                    {
                        ...
                        if (cellColorsByValue.ContainsKey(value))
                        {
                            matrixTest[r, c].GetComponent<Image>().color = cellColorsByValue[value];
                            matrixTest[r, c].transform.GetChild(1).gameObject.SetActive(true);
                            matrixTest[r, c].transform.GetChild(0).GetComponent<Text>().color = textColorsByValue[value];
                        }
                        else
                        {
                            Debug.Log("ERROR: The dictionary has not the key " + value.ToString() + "!");
